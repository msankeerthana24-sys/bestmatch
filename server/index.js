...
app.post("/api/sessions", (req, res) => { const code = Math.random().toString(36).substring(2,8).toUpperCase(); db.run("INSERT INTO sessions (code, name1) VALUES (?, ?)", [code, req.body.name || "Person 1"], (err) => { if (err) return res.status(500).json({ error: err.message }); res.json({ code }); }); });
